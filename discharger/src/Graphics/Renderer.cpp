...
bool Renderer::initVideo()
{
  ...
  const Ironhead::Graphics::gres_t *res = getResolutions(true);
...
bool Renderer::setResolution(int r, bool restoreOnFailure)
{
  ...
  {
    const Ironhead::Graphics::gres_t *res = getResolutions();
    scale        = res[r].scale;
...
{
  static Ironhead::Graphics::gres_t res[]
      = {//      description, screen_w, screen_h, render_w, render_h, scale_factor, widescreen, enabled
         // 4:3
         {(char *)"---", 0, 0, 0, 0, 1, false, true},
#if defined(__VITA__)
         {(char *)"960x544", 960, 544, 960, 544, 1, true, true},
#elif defined(__SWITCH__)
         {(char *)"1920x1080", 1920, 1080, 960, 540, 2, true, true},
#else
         {(char *)"960x544", 960, 544, 960, 544, 1, true, true},
#endif
...
void Renderer::showLoadingScreen()
{
  ...
  if (!loading.loadImage(ResourceManager::getInstance()->getLocalizedPath("Pbm/Loading2.bmp")))
    return;
...
void Renderer::drawSurface(Surface *src, int dstx, int dsty, int srcx, int srcy, int wd, int ht)
{
  ...
  SDL_SetTextureColorMod(src->texture(), src->r, src->g, src->b);
...
void Renderer::drawSurfaceEx(Surface *src, int dstx, int dsty, int srcx, int srcy, int wd, int ht, float angle)
{
  assert(_renderer);
  assert(src->texture());
  ...
  SDL_SetTextureAlphaMod(src->texture(), src->alpha);
  SDL_SetTextureColorMod(src->texture(), src->r, src->g, src->b);
  if (SDL_RenderCopyEx(_renderer, src->texture(), &srcrect, &dstrect, angle, NULL, SDL_FLIP_NONE))
  {
    LOG_ERROR("Renderer::drawSurface: SDL_RenderCopy failed: {}", SDL_GetError());
  }
}
