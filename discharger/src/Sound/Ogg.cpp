...
{
  std::string filename = ResourceManager::getInstance()->getLocalizedPath(dir + fname + ".ogg");
  _song.intro = NULL;
...
bool Ogg::start(const std::string &fname, const std::string &dir, int startbeat, bool loop, bool doloop)
{
  ...
  Mix_VolumeMusic((double)_song.volume * (double)Settings::getInstance()->musicVolume());
